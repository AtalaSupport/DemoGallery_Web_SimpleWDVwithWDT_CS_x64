# SimpleWdvWithWDT
This is a 64 bit .NET Framework 4.6.2 HTML 5 Web app with minimal Atalasoft references set.  
...
- we bound a handler to documentsaved which just pops up an alert about save status. Customers often ask about how to know if the save worked or not, thus this simple addition. It is not required
- We make a call to `_viewer.annotations.setStamps(...)` to show how to configure the "Stamp Annotation". Configuring image annotations requires a simimar set of calls, you can read more in [setStamps](https://atalasoft.github.io/web-document-viewer/Atalasoft.Controls.WebDocumentViewer-AnnotationController.html#setStamps) and [setImages](https://atalasoft.github.io/web-document-viewer/Atalasoft.Controls.WebDocumentViewer-AnnotationController.html#setImages)
- We've provided two functions related to annotations: burnAllAnnosOnPage() and burnAllAnnos(). These functions illustrate the concept of querying the annotation controller to get the annotation objects and perform a simple modification, to wit: setting the annotation burn flag true or false programmatically. Its a great example of the basis for how to walk all annotations on a page or get to all annotations on all pages programmatically to perform an action. The Burn flag affects saving. If an annotation is present and the burn flag is false or missing, saving the document will save the annotations to an xmp sidecar file. If its burn is true, the anntation will be burned into the saved image. NOTE that burning to PDF requires additional licensing - see below.
